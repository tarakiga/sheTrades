...
- `[~]` HOSTS: public/admin hostname split. The console and the privacy policy shared a hostname, and the bot mails that policy URL to every participant - so the console's address, and an invitation into its sign-in form, was published to the whole programme. Next.js middleware now serves the console only on admin.shetrades.digital (plus *.vercel.app and localhost, so DNS trouble cannot lock the team out); every other host gets /privacy and /c/... and a plain 404 for the rest, never a redirect. /previews is now absent from deployed builds entirely: it sat outside the (admin) route group and server-renders every admin workspace inline, so an auth gate could not help - curl read the whole component library out of a page that showed a sign-in prompt in the browser. 23 new tests; the whole host/path matrix verified against a local production build. The two participant-facing URLs are already republished and verified through the bot. NOT CUT OVER: needs the DNS record, the Vercel domain, ADMIN_HOSTS, and the Cloud Run env, in that order - see docs/public-admin-hostname-split.md.
