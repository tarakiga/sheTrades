...
- `[x]` CERT Phase 2: /certificates/template canvas editor. Upload artwork (raw-body route, MIME allowlist corroborated against what sharp decodes, 5 MB cap, keys never overwritten), drag fields, preview, publish, roll back. Authors the same certificate.template document, so it keeps version history and audit. The preview is the SERVER render, downscaled for transport. 37 backend + 22 dashboard tests. Deployed to staging rev 00121-d2w and verified against the live template.
- `[x]` PRIVACY: consent gate and erasure on request. Language moves first so she can read the notice before agreeing to it; the decision is recorded append-only with the published version of the notice she saw, because the notice is editable and "she agreed" without it says nothing. Erasure is one transaction with the rewards ledger kept de-identified, reachable from the bot menu and from the learner drawer. Verified end to end on staging rev 00123-2s2. OPEN: the client is replacing the notice wording, the full privacy policy has not been updated to match, and there is still no retention period.
- `[x]` HOSTS: public/admin hostname split. The console and the privacy policy shared a hostname, and the bot mails that policy URL to every participant - so the console's address, and an invitation into its sign-in form, was published to the whole programme. Next.js middleware now serves the console only on admin.shetrades.digital (plus *.vercel.app and localhost, so DNS trouble cannot lock the team out); every other host gets /privacy and /c/... and a plain 404 for the rest, never a redirect. /previews is now absent from deployed builds entirely: it sat outside the (admin) route group and server-renders every admin workspace inline, so an auth gate could not help - curl read the whole component library out of a page that showed a sign-in prompt in the browser. 23 new tests; the whole host/path matrix verified against a local production build. The two participant-facing URLs are already republished and verified through the bot. CUT OVER 2026-08-22: dashboard 37cb2c9, backend rev 00124-w25, matrix verified against production. ADMIN_DASHBOARD_URL had never been set on Cloud Run, so invite emails were falling back to the first CORS origin and briefly carried a dead link; fixed. Follow-ups closed the same day: the handbook moved out of public/ and is served through /api/handbook behind a session check (blob-URL iframe, because an iframe cannot set an Authorization header), and the public hosts came out of the CORS allowlist after confirming /privacy makes no browser call to the backend. See docs/public-admin-hostname-split.md.
- `[x]` CERT LIVE: first certificate issued end to end on 2026-08-22 (43 lessons, 5 modules). PUBLIC_BASE_URL flipped to https://www.shetrades.digital on rev 00126-zbg, held until a real certificate had been fetched through the Vercel /c/ proxy byte-identically - the .png is the URL Meta fetches, so a synthetic check would not have proved the delivery path. Pre-flip links still resolve. Both milestone rewards failed with 'Destination Not Supported' - an intentional negative test against an Oman number, and it passed: real provider rejection, retried, reason recorded. OPEN: the POSITIVE airtime path has still never succeeded end to end; it needs a Nigerian number and a funded Africa's Talking wallet.
...
## ✅ RESOLVED 2026-09-12: Meta business verification and real-number go-live

- `[x]` GO-LIVE: business verified (`business_verification_status: verified`); real number +234 803 512 5590 registered on Cloud API as `1305193106015778` on a new WABA `1727580908454655` ("SheTrades Digital by TechHer"); app subscribed for webhooks (was missing - the silent failure the runbook predicted); config published v9; bot replied through the new number. Display name chosen after checking Meta's guidelines against the CAC name TECH PROJECT WOMEN INITIATIVE LTD/GTE - techherng.com footer and Digital Literacy page updated so the brand-to-legal-name chain is public. OPEN: payment method on the new WABA, display-name review outcome, reset test learners, regenerate QR, keep the test number as rollback. Full record at the top of `docs/whatsapp-number-golive-runbook.md`.

### (superseded) LAUNCH BLOCKER as recorded 2026-08-17
...
- `[x]` TECHHER: complete business verification (done - see GO-LIVE above) (Business portfolio -> WhatsApp account -> Start Verification). Needs CAC certificate + proof of address + utility bill/bank statement matching the registered name. THIS NOW BLOCKS EVERYTHING - number registration, display name, and higher messaging limits all depend on it.
