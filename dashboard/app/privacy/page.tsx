...
// Editable defaults. Prefer managing these (and the full policy body) through the
// legal config namespace; these are the safe fallbacks used when it is unpopulated.
const ORG_NAME = "SheTrades Digital";
const CONTACT_EMAIL = "privacy@shetrades.digital"; // replace with the client's real privacy contact
const EFFECTIVE_DATE = "13 July 2026";

// If a legal document with a key starting with this prefix is published, its text
// overrides the built-in default below (per the data-driven content mandate).
const POLICY_KEY_PREFIX = "legal.privacy.";
...
export default async function PrivacyPolicyPage() {
  // Prefer admin-managed legal copy; fall back to the safe built-in default.
  let override: string | null = null;
  try {
    const result = await getPublicConfigNamespace("legal");
    const doc = result.data.documents.find(
      (item) =>
        item.key.startsWith(POLICY_KEY_PREFIX) &&
        typeof item.data.en === "string" &&
        (item.data.en as string).trim().length > 0
    );
    if (doc) override = doc.data.en as string;
  } catch {
    override = null;
  }
    ...
    <main className="legal-page">
      <article className="legal-page__inner">
        <header className="legal-page__header">
          <p className="legal-page__eyebrow">{ORG_NAME}</p>
        ...
        {override ? (
          <section className="legal-page__section">
            {paragraphsFrom(override).map((paragraph, index) => (
